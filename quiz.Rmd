...
Which range has the highest frequency of `lender_count` in Indonesia?
...
For exploratory purposes, find out the range of the `posted_time`:
...
Since we want to visualize the monthly average funding duration, we need to create a new column called `month` which contains the month of the `posted_time` before aggregating the data. 
```{r}
# your code here
```
